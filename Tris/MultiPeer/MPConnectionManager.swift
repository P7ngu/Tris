...
extension String {
    static var serviceName = "XAndO" //It must match the p-list entry
}

class MPConnectionManager: NSObject, ObservableObject { //Objective C framework + must update UI accordingly
...
extension MPConnectionManager: MCNearbyServiceBrowserDelegate {
    func browser(_ browser: MCNearbyServiceBrowser, foundPeer peerID: MCPeerID, withDiscoveryInfo info: [String : String]?) {
        //Update a list that we can pick from
...
extension MPConnectionManager: MCSessionDelegate {
    ...
    func session(_ session: MCSession, didReceive data: Data, fromPeer peerID: MCPeerID) {
        //DID RECEIVE, so we need to answer back
